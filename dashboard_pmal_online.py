...
# Função para limpar nome das OPMs
...
def main():
    ...
    cidades_opm_file = st.sidebar.file_uploader(
        "4️⃣ Selecione o arquivo de cidades por OPM (.xlsx) [opcional, mas necessário para o painel de redistribuição]",
        type="xlsx", 
        accept_multiple_files=False
    )
    ...
    # Carrega arquivo de cidades/OPMs do interior
    if cidades_opm_file:
        ...
        if not 'OPM' in df_cidades_opm.columns or not 'QTD_CIDADES' in df_cidades_opm.columns:
            st.error("O arquivo de cidades precisa ter as colunas 'OPM' e 'QTD_CIDADES'.")
            st.stop()
    ...
    if df_cidades_opm is not None:
        ...
        # Junta com o dataframe de cidades
        ...
        st.info(
            "🔎 **Menor valor de Viaturas por Cidade indica OPM potencialmente mais desfavorecida** (menos viaturas para atender mais cidades)."
        )
